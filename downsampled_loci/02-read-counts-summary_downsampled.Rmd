...
# feature table df
feature_df_no_eDNA <- readRDS("../extdata/downsampled_loci/data/downsampled_features_df.rds")

# taxonomy table df
taxonomy_df  <- readRDS("../extdata/downsampled_loci/data/taxonomy_ASV_BLAST_hits_fixed_accession.rds")
...
uncollapsed_taxonomy_spp_98 <- read_rds("../extdata/downsampled_loci/data/uncollapsed_taxonomy_spp_98.rds")
unique_taxonomy_spp_98 <- read_rds("../extdata/downsampled_loci/data/unique_taxonomy_spp_98.rds")
...
ggsave("pdf_outputs/downsampled_SI_figure_reads_per_locus_boxplot.pdf", width = 8, height = 5)
```
the only loci with > 100,000 reads are those with multiple primers contributing (i.e., aquaF3a and aquaF3b were downsampled separately)


Same figure, but keeping the y-axis consistent with the previous (non-downsampled) figure
```{r}
feature_df_no_eDNA %>% 
  filter(str_detect(sample, "RP")) %>% # just the reference pools
  group_by(locus) %>%
...
ggsave("pdf_outputs/downsampled_SI_figure_reads_per_locus_boxplot_scaled.pdf", width = 8, height = 5)
...
Average reads per locus per sample
```{r mean-reads-per-locus-per-sample}
feature_df_no_eDNA %>%
  filter(!str_detect(sample, "F1") & !str_detect(sample, "F2")) %>% # not the feeds
  filter(!locus %in% c("16Sfish", "teleo", "crust2")) %>% # remove the low-read count loci
  left_join(., unique_taxonomy_spp_98) %>%
  filter(!is.na(taxon)) %>%
  filter(!taxon %in% c("Eukaryota" ,"Homo_sapiens", "Homo", "Arthropoda", "Mammalia")) %>%
  unique() %>%
  group_by(locus, seq, sample) %>%
  slice_max(count) %>%
  group_by(locus, sample) %>%
  summarise(reads = sum(count)) %>%
  mutate(sample_type = ifelse(str_detect(sample, "EXB"), "extraction_blank", "sample")) %>%
  mutate(sample_type = ifelse(str_detect(sample, "NEG"), "pcr_negative", sample_type)) %>%
  group_by(locus, sample_type) %>%
  summarise(mean_reads = mean(reads)) %>%
  ggplot(aes(x = locus, y = mean_reads, fill = sample_type)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 0.95)
  )
...
features_w_taxonomy %>%
  saveRDS("../extdata/downsampled_loci/data/feature_df_no_eDNA_metazoa_only.rds", compress = "xz")
